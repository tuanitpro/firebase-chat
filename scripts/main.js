...
FriendlyChat.prototype.initFirebase = function () {
  ...
  this.roomRef = this.database.ref('rooms');
};
...
FriendlyChat.prototype.createRoom = function () {
  if (this.checkSignedInWithMessage()) {
    ...
    if (name) {
      ...
      this.roomRef.child(key).child('meta_data').set({
        name: name,
        type: 'public',
...
this._rooms=[];
  ...
  if (this.checkSignedInWithMessage()) {
    this.roomRef.once('value', function (snapshot) {
      ...
      for (var roomId in rooms) {
        ...
        var item = rooms[roomId].meta_data;
        item.id = roomId;
  ...
  var html = '<h3>Rooms</h3>';

  for (var item in this._rooms) {
    console.log(this._rooms[item]);
    html += '<a href="#">' + this._rooms[item] + '</a>&nbsp;';
  }
  console.log(html);
...
FriendlyChat.prototype.loadMessages = function () {
  ...
  this.messagesRef = this.roomRef.child(roomId).child('messages');
...
FriendlyChat.prototype.onAuthStateChanged = function (user) {
  if (user) { // User is signed in!
    ...
    this.usersRef = this.roomRef.child(roomId).child('users').child(userkey);
...
FriendlyChat.prototype.displayMessage = function (key, dataItem) {
  ...
  if (dataItem.text) { // If the message is text.
    messageElement.textContent = dataItem.text;
